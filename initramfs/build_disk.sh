...
if [ "${ROOTFS_CONTENT}" = "BUILDROOT" ]; then
	...
	# === Write OS version to /etc/os_version.txt===
	LINUX_DTB_SUFFIX=$(echo "${LINUX_DTB}" | sed 's/^eys3d\/ecv5546-//' | tr '[:lower:]' '[:upper:]')
	ROOTFS_CONTENT_UPPER=$(echo "${ROOTFS_CONTENT}" | tr '[:lower:]' '[:upper:]')
	OS_VERSION="${CHIP}_${LINUX_DTB_SUFFIX}_${BOOT_FROM}_${ROOTFS_CONTENT_UPPER}_${GIT_TAG}_${GIT_COMMIT}"
	echo "${OS_VERSION}" > ${DISKOUT}/etc/os_version.txt
